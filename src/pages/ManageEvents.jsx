...
const EditEventModal = ({ event, isOpen, onClose, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div
                className="bg-base-100 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col"
            ...
            >
                ...
                <footer className="flex-shrink-0 p-6 border-t border-slate-200 flex justify-end gap-4">
                    <button onClick={onClose} className="px-6 py-2 rounded-lg font-semibold text-primary-content bg-slate-100 hover:bg-slate-200 transition-colors">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            >
                ...
                <div className="flex justify-center gap-4 mt-8">
                    <button onClick={onClose} className="px-8 py-2 rounded-lg font-semibold text-primary-content bg-slate-100 hover:bg-slate-200 transition-colors">Cancel</button>
...
const EventCard = ({ event, onEdit, onDelete }) => (
    <motion.div
        ...
        className="bg-base-100 rounded-2xl shadow-sm overflow-hidden flex flex-col group transition-all hover:shadow-xl hover:-translate-y-1"
    >
        <div className="p-6 flex-grow">
            <div className="flex justify-between items-start mb-4">
                <span
                    className={`inline-block px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full ${event.status === 'Upcoming' ? 'bg-blue-100 text-blue-800' : 'bg-gray-200 text-gray-700'
                        }`}
                >
                    {event.status}
                </span>
            ...
            <div className="space-y-2 text-sm">
                ...
                <p className="flex items-center gap-2"><Calendar className="w-4 h-4" /> {new Date(event.date).toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
            </div>
        </div>
        <footer className="bg-base-100 p-4 border-t border-slate-100 flex justify-between items-center">
            <Link to={'event/:id'} className="inline-flex items-center gap-2 text-sm font-semibold hover:underline">
                <Eye className="w-4 h-4" /> View Details
            </Link>
            <div className="flex gap-2">
                <button onClick={() => onEdit(event)} className="p-2 hover:bg-slate-200 hover:text-slate-800 rounded-md transition-colors">
...
const EventRow = ({ event, onEdit, onDelete }) => (
    <motion.div
        ...
        exit={{ opacity: 0 }}
        transition={{ duration: 0.3 }}
        className="grid grid-cols-1 md:grid-cols-8 gap-4 items-center p-4 bg-base-100 rounded-lg shadow-sm hover:bg-base-300 transition-colors duration-200 group"
    >
        ...
        <div className="md:col-span-2">
            <p className="font-semibold ">{new Date(event.date).toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
        ...
        <div className="bg-base-200 min-h-screen antialiased">
            <div className="container mx-auto px-6 py-16 md:py-24">
                <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
                    <div>
                        <h1 className="text-4xl font-extrabold">Manage My Events</h1>
                        <p className="text-lg mt-1">View, edit, or delete the events you've created.</p>
                    </div>
