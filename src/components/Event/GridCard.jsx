...
const GridCard = ({ event }) => {
    ...
    return (
        <motion.div
            ...
            className="bg-base-100 rounded-2xl shadow-sm overflow-hidden group transform hover:-translate-y-1 transition-transform duration-300 flex flex-col"
        >
            ...
            <div className="p-5 flex flex-col flex-grow">
                 <p className={`inline-block px-3 py-1 text-xs font-semibold rounded-full self-start mb-3 ${eventTypeColors[event.type] || 'bg-gray-100'}`}>{event.type}</p>
                <h3 className="text-lg font-bold mb-2 flex-grow">{event.title}</h3>
                <p className="flex items-center text-sm mb-4">
                    <MapPin className="w-4 h-4 mr-2 flex-shrink-0" /> {event.location}
                </p>
                <div className="flex justify-between items-center mt-auto pt-4 border-t border-slate-100">
                    <p className="font-bold ">{new Date(event.date).toLocaleDateString('en-GB', { month: 'short', day: 'numeric' })}</p>
